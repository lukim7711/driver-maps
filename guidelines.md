...
- **TSP Router Solver:** Pemecah rute pintar berada pada `backend/services/maps.js` (`optimizeSmartRoute`) secara lokal menggunakan **2-phase brute-force permutasi**. Fase 1: brute-force urutan pickup optimal dari posisi driver. Fase 2: brute-force urutan delivery optimal dari pickup terakhir. Norma lapangan: pickup semua dulu, baru delivery. Kompleksitas 2×N! (bukan (2N)!): untuk 6 struk, 2×720=1.440 permutasi (milidetik di Node.js). Selalu menemukan global optimal untuk norma pickup-all-first. Jarak Haversine digunakan sebagai estimasi jarak antar titik tanpa perlu panggil Route Matrix API saat solving.
