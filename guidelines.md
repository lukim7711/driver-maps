...
- **TSP Router Solver:** Pemecah rute pintar berada pada `backend/services/maps.js` (`optimizeSmartRoute`) secara lokal menggunakan **hybrid solver dengan 2-opt**. Untuk ≤3 struk: brute-force semua permutasi dengan constraint pickup-before-delivery (720 permutasi, global optimal). Untuk >3 struk: Greedy Nearest Neighbor heuristic diikuti 2-opt local search untuk menghilangkan crossing edges dan memperbaiki route quality. Norma lapangan: pickup semua dulu, baru delivery. Kompleksitas: O(N²) per iterasi 2-opt. Jarak Haversine digunakan sebagai estimasi jarak antar titik tanpa perlu panggil Route Matrix API saat solving.
