...
- **TSP solver is hybrid with 2-opt** (`maps.js:optimizeSmartRoute`) — For ≤3 orders: brute-force all permutations with pickup-before-delivery constraint (720 permutations, global optimal). For >3 orders: Greedy Nearest Neighbor heuristic followed by 2-opt local search to eliminate crossing edges and improve route quality. Uses local Haversine distance matrix for speed; no Route Matrix API call needed during solving. Pickup-all-first norm validated by field research.
